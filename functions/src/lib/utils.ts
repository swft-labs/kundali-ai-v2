...
export async function retrieveKundaliSummary(
  db: admin.firestore.Firestore,
  uid: string,
) {
...
export async function retrieveBirthDetails(
  db: admin.firestore.Firestore,
  uid: string,
) {
  try {
...
export async function callAstrologyAPI(
  db: admin.firestore.Firestore,
  uid: string,
  endpoint: string,
): Promise<any> {
  try {
    ...
    const response = await axios.post(
      `${process.env.ASTROLOGY_API_BASE_URL}/${endpoint}`,
      requestData,
      {
        headers: {
  ...
  return {
    ...
    muhurta: {
      ...
      yamghant_kaal: response.yamghant_kaal
        ? {
            start: response.yamghant_kaal.start,
            end: response.yamghant_kaal.end,
          }
        : null,
    },
  };
...
export async function getPanchangData(
  ...
  try {
    ...
    const response = await axios.post(
      `${process.env.ASTROLOGY_API_BASE_URL}/advanced_panchang`,
      requestData,
      {
        headers: {
          Authorization: `Basic ${Buffer.from(`${process.env.ASTROLOGY_API_USER_ID}:${process.env.ASTROLOGY_API_KEY}`).toString("base64")}`,
    ...
    console.error(
      `Error fetching Panchang data from advanced_panchang:`,
      error,
    );
    throw error;
  }
...
export async function retrieveSocialProfile(
  db: admin.firestore.Firestore,
  uid: string,
) {
